...
public class gameover {
	private int x;
	private int y;
	private Bitmap grade,bmover1,bmover2,bmover3;
	...
	private int maxf;
	private int count = 0;
   public gameover(Bitmap grade,Bitmap bmover1,Bitmap bmover2,Bitmap bmover3,int x,int y)
   {   maxf = 5;
	   ...
	   framew3 = (int)bmover3.getWidth();
	   frameh3 = (int)bmover3.getHeight();
   ...
   public void grade(Canvas canvas,Paint paint){
	   canvas.drawBitmap(grade,x-grade.getWidth(),y-grade.getHeight(),paint);
	   canvas.drawText(String.valueOf(mysurfaceview.gametime), x+20, y-grade.getHeight()/3, paint);
   }
	  ...
	  // canvas.save();
	   if(flag==0||flag==2){
		   ...
		   Bitmap dstbmp = Bitmap.createBitmap(bmover2, 0, 0, bmover2.getWidth(), bmover2.getHeight()   ,  
	               mat, true);  
		   canvas.drawBitmap(dstbmp, x-fw2/2, y+y/10, paint);
	   }
	   if(flag==1){
		   ...
		   Bitmap dstbmp = Bitmap.createBitmap(bmover2, 0, 0, bmover2.getWidth(), bmover2.getHeight()   ,  
	               mat, true);  
		   canvas.drawBitmap(dstbmp, x-fw2/2+fw2/20, y+y/10+fh2/20, paint);
   ...
   {		mx=(int)event.getX();
	   ...
	   if(flag==2&&event.getAction()==MotionEvent.ACTION_UP)
	   {
		   flag=0;
		   mysurfaceview.bb=1;
